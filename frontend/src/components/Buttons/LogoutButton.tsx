...
import { useDispatch, useSelector } from "react-redux";
import { useNavigate } from "react-router-dom";
import { selectIsLoggedIn } from "../../redux/slices/authSlice"; // Используем селектор для получения токена и состояния логина
import { AppDispatch } from "../../redux/store";
import { logoutUser } from "../../redux/operations"; // Используем logoutUser как экшен
...
const LogoutButton = () => {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Вызываем экшен logoutUser из Redux
      await dispatch(logoutUser()).unwrap();

      console.log("Логаут успешно выполнен");

      // После успешного logout перенаправляем пользователя на страницу входа
      navigate("/login");
    } catch (error) {
      console.error("Ошибка при выполнении логаута", error);
    } finally {
